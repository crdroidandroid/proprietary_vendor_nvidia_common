...
class TFlashT23x_Base(object):
    ...
    def tegraflash_oem_enc(self, filename, bct_flag = False, compress = 0, meta_blob_sz = 0):
        ...
        lines += 'CHIPID : "%s"\n' %(chip_info)
        if values['--iv'] == 'RANDOM':
            lines += 'RANDOM : " IV "\n' # Current, self provided IV is not supported
    ...
    def tegraflash_oem_sign_file(self, in_file, magic_id):
        ...
        if filename == "mb1_bct_MB1_aligned.bct":
            info_print('Generating SHA2 Hash for mb1bct')

            mb1size = os.path.getsize(filename)
            mb1offset = 0
            sha_offset = mb1size
            sha_size = 64
            ...
            with open(filename, 'rb+') as f:
                src = bytearray(f.read())
                sha_file_name = compute_sha('sha512', filename, mb1offset, mb1size)
                # Append to end of binary
                with open(sha_file_name, 'rb') as sha_f:
                    src[sha_offset:sha_offset+sha_size] = bytearray(sha_f.read())
                f.seek(0)
        ...
        if values['--minratchet_config'] is not None:
            ...
            command.extend(['--ratchet_blob',
                            self.tegrahost_values['--ratchet_blob']])
        # Special handling for dce since it needs compression
        if (magic_id == 'DCEF'):
    ...
    def tegraflash_write(self, args, partition_name, filename):
        ...
        elif 'dce-fw' in partition_name:
            # Need to compress and add header so use oem_sign to handle
            filename = self.tegraflash_oem_sign_file(filename, 'DCEF')
        self.tegraflash_erase_partition(partition_name)
    ...
    def tegraflash_encrypt_sign_binary(self, exports, args):
        ...
        if (magicid == 'FSKP'):
            return self.tegraflash_oem_sign_file(file_path, magicid)
